...
$sql="
 CREATE  TABLE  `lesjardinsdecocagnech`.`jos_pdds` (  `id` int( 12  )  NOT  NULL  AUTO_INCREMENT ,
...
$sql="INSERT INTO `lesjardinsdecocagnech`.`jos_pdds` SELECT * FROM `lesjardinsdecocagnech`.`jos_pddsSAV`";
$sql=mysql_query($sql);
if(!$sql) {
	echo "Erreur MySQL:<br>" .mysql_error(); exit;
}
...
while($i<mysql_num_rows($sql)){
	...
	if(mysql_num_rows($ssql)!=1) {
		...
		if(mysql_result($sql,$i,'PDDTexte')!=utf8_decode(mysql_result($ssql,0,'PDDTexte'))) {
			...
			$update=mysql_query("UPDATE jos_pdds SET PDDTexte='".(mysql_result($sql,$i,'PDDTexte')) ."' WHERE PDDINo=".$PDDINo);
		}
		...
		if(mysql_result($sql,$i,'PDDNom')!=utf8_decode(mysql_result($ssql,0,'PDDNom'))) {
			...
			$update=mysql_query("UPDATE jos_pdds SET PDDNom='".(mysql_result($sql,$i,'PDDNom')) ."' WHERE PDDINo=".$PDDINo);
		}
		...
		if(mysql_result($sql,$i,'PDDAdr')!=utf8_decode(mysql_result($ssql,0,'PDDAdr'))) {
			...
			$update=mysql_query("UPDATE jos_pdds SET PDDAdr='".(mysql_result($sql,$i,'PDDAdr')) ."' WHERE PDDINo=".$PDDINo);
		}
		...
		if(mysql_result($sql,$i,'PDDNoRue')!=mysql_result($ssql,0,'PDDNoRue')) {
			...
			$update=mysql_query("UPDATE jos_pdds SET PDDNoRue='".(mysql_result($sql,$i,'PDDNoRue')) ."' WHERE PDDINo=".$PDDINo);
		}
		...
		if(mysql_result($sql,$i,'PDDTele')!=mysql_result($ssql,0,'PDDTele')) {
			...
			$update=mysql_query("UPDATE jos_pdds SET PDDTele='".(mysql_result($sql,$i,'PDDTele')) ."' WHERE PDDINo=".$PDDINo);
		}
		#PDDLieu 
		if(mysql_result($sql,$i,'PDDLieu')!=utf8_decode(mysql_result($ssql,0,'PDDLieu'))) {
			...
			$update=mysql_query("UPDATE jos_pdds SET PDDLieu='".(mysql_result($sql,$i,'PDDLieu')) ."' WHERE PDDINo=".$PDDINo);
		}
		...
		if(mysql_result($sql,$i,'PDDEmail')!=mysql_result($ssql,0,'PDDEmail')) {
			...
			$update=mysql_query("UPDATE jos_pdds SET PDDEmail='".(mysql_result($sql,$i,'PDDEmail')) ."' WHERE PDDINo=".$PDDINo);
		}
		#PDDRem 
		if(mysql_result($sql,$i,'PDDRem')!=utf8_decode(mysql_result($ssql,0,'PDDRem'))) {
			...
			$update=mysql_query("UPDATE jos_pdds SET PDDRem='".(mysql_result($sql,$i,'PDDRem')) ."' WHERE PDDINo=".$PDDINo);
		}
		...
		if(mysql_result($sql,$i,'PDDGP')!=mysql_result($ssql,0,'PDDGP')) {
			...
			$update=mysql_query("UPDATE jos_pdds SET PDDGP='".(mysql_result($sql,$i,'PDDGP')) ."' WHERE PDDINo=".$PDDINo);
		}
		#PDDPP 
		if(mysql_result($sql,$i,'PDDPP')!=mysql_result($ssql,0,'PDDPP')) {
			...
			$update=mysql_query("UPDATE jos_pdds SET PDDPP='".(mysql_result($sql,$i,'PDDPP')) ."' WHERE PDDINo=".$PDDINo);
		}
	}
